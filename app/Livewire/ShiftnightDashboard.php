...
class ShiftnightDashboard extends Page implements HasInfolists
{
    use InteractsWithInfolists;

    protected static ?string $navigationIcon = 'heroicon-o-clock';
    
    protected static string $view = 'livewire.shiftnight-dashboard';
    ...
    public function loadShiftRecord(): void
    {
        if ($this->filterType === 'specific' && $this->selectedDate) {
            ...
            $this->shiftRecord = Shiftnight::whereDate('day', Carbon::parse($this->selectedDate))->first();
        } elseif ($this->filterType === 'next') {
            // Get next upcoming shift
            $this->shiftRecord = Shiftnight::where('day', '>=', now())
                ->orderBy('day', 'asc')
                ->first();
    ...
    public function getShiftnightInfolistProperty(): Infolist
    {
    ...
    Carbon::parse($state)
        ...
        ->translatedFormat('l, d F Y - H:i')
                        ...
                        Infolists\Components\TextEntry::make('break_1 (15:00-16:00)')
                            ->label('Break 1')
                            ->placeholder('None')
