...
class ShiftNightsTable extends TableComponent implements HasTable
{
    ...
    {
        return $table
            ->query(fn (): Builder => Shiftnight::query()->latest('id'))
            ->selectable(false)
            ->columns([
                TextColumn::make('day')
                    ->label('Hari/Tanggal')
            ...
            ->actions([
                Tables\Actions\Action::make('edit')
                    ...
                    ->color('warning')
                    ->url(fn ($record) => route('filament.admin.resources.shiftnights.edit', $record)),
                DeleteAction::make()
                    ->label('Delete')
    ...
    {
        return view('livewire.shiftnights-table');
    }
